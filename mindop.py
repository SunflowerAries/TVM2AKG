...
class Statement:
    def __init__(self, cnt, axes, kind, has_pad=False, has_unpad=False, backbone="Conv2D", has_transpose=False, has_image_pad=False):
        self.cnt = cnt
        self.axes = axes
        self.kind = kind
        self.has_pad = has_pad
        self.has_image_pad = has_image_pad
    ...
    def coefficients(self):
        if self.kind in ["Conv2D", "MatMul", "BatchMatMul"]:
            coeffs = []
            if self.has_pad or self.has_image_pad:
                tmp = [0] * (len(self.axes) + 1)
                tmp[-1] = 1
                if self.has_pad and self.has_image_pad:
                    tmp[-1] = 2
            ...
            coeffs = []
            tmp = [0] * (len(self.axes) + 1)
            if self.has_image_pad and self.has_pad:
                tmp[-1] = 1
            coeffs.append(str(tmp))
            for i in range(len(self.axes)):
                tmp = [0] * (len(self.axes) + 1)
        ...
        if self.backbone == "Conv2D":
            ...
            for op in json_obj["op_desc"]:
                if op["name"] == "PadAkg":
                    ...
                    has_image_pad = any(ps != 0 for ps in pad)
                    if any(ps != 0 for ps in pad):
                        padded_size = copy.deepcopy(image_size)
                        padded_size[1] += (pad[0] + pad[2])
                        padded_size[2] += (pad[1] + pad[3])
                        self.statements.append(Statement(cnt, padded_size, "Pad", has_pad, has_image_pad=has_image_pad))
                        cnt += 1
                    self.kh_kw = len(list(filter(lambda size: size != 1, kernel_size)))
                    self.statements.append(Statement(cnt, output_size, "Init", has_pad, backbone="Conv2D", has_image_pad=has_image_pad))
                    cnt += 1
                    self.statements.append(Statement(cnt, output_size + [image_size[3]] + list(filter(lambda size: size != 1, kernel_size)), "Conv2D", has_pad, has_image_pad=has_image_pad))
                    cnt += 1
                else:
                    self.statements.append(Statement(cnt, output_size, "Elem", has_pad, backbone="Conv2D", has_image_pad=has_image_pad))
                    return
            ...
            for op in json_obj["op_desc"]:
                if op["name"] == "PadAkg":
                    ...
                    tensor_a = op["input_desc"][0][0]["shape"]
                    tensor_c = op["output_desc"][0]["shape"]
                    self.statements.append(Statement(cnt, tensor_c, "Init", has_pad, backbone="BatchMatMul", has_transpose=has_transpose))
                    cnt += 1
                    ...
                    self.statements.append(Statement(cnt, tensor_c, "Elem", has_pad, backbone="BatchMatMul", has_transpose=has_transpose))
                    cnt += 1
                    break
